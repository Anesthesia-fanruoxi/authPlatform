...
fn route(app: &AppHandle, url: &str, method: &Method, body: &[u8]) -> (u16, String) {
    ...
    if url.starts_with("/identity") {
        ...
        return match &*sess {
            Some(s) => (
                200,
                serde_json::json!({"logged_in": true, "user": {"uid": s.uid, "username": s.username, "nickname": s.nickname}}).to_string(),
            ),
            None => (200, serde_json::json!({"logged_in": false}).to_string()),
        };
    }
        ...
        return match serde_json::from_slice::<serde_json::Value>(body) {
            Ok(v) => {
                ...
                if request_id.is_empty() {
                    return (400, serde_json::json!({"code": 1, "msg": "request_id 为空"}).to_string());
                }
