...
# -*- coding: utf-8 -*-
"""
桌面令牌登录流程自测脚本（模拟桌面端内部逻辑）
================================================
新架构（2026-08）：平台 → 桌面端 → 认证中心。
本脚本模拟**桌面端**替平台完成的内部流程（initiate → 确认 → exchange），
用于开发/联调时验证认证中心侧链路，不面向平台接入。

平台接入请直接调桌面端本地接口：
  POST http://127.0.0.1:5712/login  {"platform_id":"你的平台ID"}
  → 桌面端内部用该平台的 secret 配置签名 initiate → 弹确认窗 → confirm → exchange
  → 同步返回 {"code":0,"token":"...","user":{...}}
完整平台接入示例见 platform_integration.md。

依赖：pip install requests
注意：桌面端本地服务 `/login` 接口为本架构的配套实现（见 desktop/ 源码）。
"""
import hashlib
import hmac
import json
