...
mux.HandleFunc("PUT  /api/admin/users/{id}", adminAuth(users, secret, s.UpdateUser)) // 路径参数 r.PathValue("id")
...
| 敏感存储 | 平台盐明文存储（字段名 secret_enc 保留历史命名），列表接口只返回脱敏前缀 |
