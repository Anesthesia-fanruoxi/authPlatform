...
## 8. 桌面令牌登录（Tauri 客户端 + 平台免密）
## 8. 桌面令牌登录（平台 → 桌面端 → 认证中心）

> 面向场景：用户已登录桌面客户端（`desktop/`，Tauri），访问各平台时选择「令牌登录」，在客户端弹窗确认后免密进入。
> **职责边界（2026-08 修订）**：登录环节平台**零配置、零签名**，只调桌面端本地接口；桌面端持有各平台在认证中心的登录代理配置（platform_id + secret），封装 initiate/confirm/exchange 全部签名逻辑；认证中心做身份确认与 token 签发。
> 好处：① 所有平台都能通过桌面端快捷登录；② 登录签名与加密盐逻辑全部封装在桌面端，平台前端/后端不接触登录敏感逻辑；③ 减少平台配置——登录环节所有配置都在桌面端。
> ⚠️ **边界**：「平台零配置」仅限登录环节；平台后端拉取/同步用户信息（§7）仍是服务器到服务器调用，**平台后端必须保留自己的 secret**。
> 完整可照抄示例见 `desktop/examples/platform_integration.md`。

### 8.1 三端架构（同步响应）

```
平台登录页(JS)                   桌面端(5712)                         认证中心
      │  ① POST /login {platform_id}  │                                  │
      │───────────────►│  (挂起等待)                                     │
      │                │  ② 签名 initiate（用该平台 secret 配置）───────────►│
      │                │  ③ 弹确认窗「平台X请求以xxx登录？确认/拒绝」         │
      │                │  ④ 用户确认 → confirm（desktop_token）───────────►│
      │                │  ⑤ exchange → 校验授权 → 签发平台 token ────────►│
      │                │◀─ token                                           │
      │ ⑥ 同步响应 {token,user}│                                           │
      │◀───────────────│                                                  │
      │ ⑦ POST /api/session {token} → 平台后端建会话（无签名）              │
```

### 8.2 接口清单

**平台 → 桌面端（平台唯一需要调用的本地接口）**

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `http://127.0.0.1:5712/login` | 发起令牌登录：`{"platform_id":"ops-platforms"}`。请求挂起至用户确认/拒绝（60s），确认后同步返回 `{"code":0,"token":"...","user":{...}}` |

**桌面端 → 认证中心（桌面端内部调用，平台不感知）**

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/auth/desktop/initiate` | 桌面端用平台 secret 签名发起，拿一次性 `request_id`（60s） |
| POST | `/api/auth/desktop/confirm` | 用户确认后绑定请求与客户端用户（desktop_token 会话） |
| POST | `/api/auth/desktop/exchange` | 兑换平台 token（校验授权，一次性） |

**客户端 → 认证中心（桌面端登录/登出）**

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/auth/desktop/login` | 客户端登录（账号/密码或 TOTP），返回 `desktop_token`（30 天，仅返回一次） |

桌面端本地服务另有：`GET /identity`（返回登录状态）、`POST /pending`（旧推送接口，新架构下由 `/login` 内部流程替代）、`GET /status`。

### 8.3 平台侧接入步骤（前端 JS + 后端建会话）

```js
// —— 平台登录页 JS：调桌面端 → 拿 token → 交后端建会话（完整代码见 platform_integration.md）——
const res = await fetch('http://127.0.0.1:5712/login', {
  method: 'POST', headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ platform_id: 'ops-platforms' }),   // 挂起至用户确认，60s
}).then(r => r.json());
if (res.code !== 0) return alert(res.msg);                  // 未检测到客户端/未登录/拒绝
await fetch('/api/session', {                               // token 仅返回一次，立即交后端
  method: 'POST', headers: { 'Content-Type': 'application/json' },
...
```python
# —— 平台后端：只收 token 建会话（无 secret、无签名）——
@app.post("/api/session")
def create_session():
    data = request.get_json()
    token, user = data.get("token"), data.get("user")
    if not token: return jsonify(ok=False, msg="缺少 token"), 400
    session["uid"], session["username"], session["auth_token"] = user["uid"], user["username"], token
    return jsonify(ok=True, user=user)
```

### 8.4 桌面端配置（登录后自动拉取授权平台）

桌面端登录认证中心后自动拉取「当前用户被授权的平台 + 各平台登录代理 secret」（认证中心配套新增接口 `GET /api/auth/desktop/platforms`，凭 `desktop_token` 会话身份），**无需手动配置**：

```
桌面端登录 → GET /api/auth/desktop/platforms → [{platform_id, name, secret}, ...]
```

> 该 secret 仅用于**登录代理**；平台后端做用户同步（§7）仍使用自己在认证中心注册的 secret。

### 8.5 安全说明

- **签名与 secret 只在桌面端**：平台页面/后端完全不接触认证中心签名与各平台 secret；secret 以桌面端配置形式本地保管（生产建议加密存储/系统 keyring）。
- **确认必须在桌面端弹出**：平台页面只能发起请求，无法替用户确认；`request_id` 一次性（confirmed→used）、60s 过期。
- **客户端身份**：`desktop/confirm` 凭 `desktop_token`（认证中心只存哈希、可吊销）证明"当前桌面用户"，认证中心校验会话有效后才绑定请求。
- **认证中心仍强制验授权**：`exchange` 时校验该用户对该平台的授权（未授权 1006），用户被禁用/删除即时拦截。
- **本地端口安全**：`127.0.0.1:5712` 仅本机可达；`/login` 只对**已登录桌面端**生效；恶意网页可触发确认弹窗骚扰，但无法替用户确认，且 token 必须由认证中心按平台授权签发。
- **desktop_token 保管**：存于客户端本机（`%APPDATA%/authplatform-desktop/session.json`），建议升级系统 keyring 加密存储。
- **平台后端信任链**：桌面端交回的 token 由认证中心签发，平台后端收到后建议通过认证中心的 token 校验接口核验（配套新增），再建立本地会话。

### 8.6 客户端使用

```bash
cd desktop/src-tauri && cargo build   # 构建
./target/debug/authplatform-desktop.exe  # 启动：登录认证中心 → 保持运行（托盘/窗口）
```
平台登录页集成：登录表单旁增加「桌面令牌登录」入口，点击后按 §8.3 流程执行。
