...
> **职责边界（2026-08 修订）**：平台**零配置、零签名**，只调桌面端本地接口；桌面端持有各平台在认证中心的接入配置（platform_id + secret），封装 initiate/confirm/exchange 全部签名逻辑；认证中心做身份确认与 token 签发。
> 好处：① 所有平台都能通过桌面端快捷登录；② 签名与加密盐逻辑全部封装在桌面端，平台不接触敏感逻辑；③ 减少平台配置，所有配置都在桌面端。
...
### 8.4 桌面端配置（替代平台侧 secret 配置）

桌面端本地 `%APPDATA%/authplatform-desktop/platforms.json` 持有各平台接入参数，平台侧不再配置：

```json
{ "auth_base": "http://authplatform.example.com",
  "platforms": [
    { "platform_id": "ops-platforms", "name": "运维平台", "secret": "…认证中心签发给该平台的 secret…" },
    { "platform_id": "data-platform", "name": "数据平台", "secret": "…" }
  ] }
```
