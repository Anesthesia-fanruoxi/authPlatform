...
> 职责边界：**平台**发起请求、组装信息到服务器验证；**客户端**被动接收平台推送的请求并展示确认界面；**认证中心**做身份确认与 token 签发。

### 8.1 三端架构

```
平台登录页(令牌登录)                      认证中心                     桌面客户端(Tauri)
      │                                   │                            │
      ├─ initiate ── request_id ────────▶ │ 生成一次性request_id(60s)    │
      │                                   │                            │
      │ 探测127.0.0.1:5712/identity ◀─────┼────────────────────────────┤ GET /identity(会话用户)
      │ POST /pending 推送 request_id ────┼───────────────────────────▶ │ 弹确认窗「平台X请求以xxx登录?」
      │                                   │                            │ 用户确认 → POST desktop/confirm
      │                                   │ ◀── confirm(request_id) ───┤ (凭 desktop_token 证明身份)
      │                                   │ 标记 confirmed+绑定用户     │
      │ ◀── poll: confirmed ───────────── │                            │
      ├─ exchange(request_id) ──────────▶ │ 校验授权 → 签发平台token(一次性)│
      ◀── token ──────────────────────────│                            │
```
...
| 端 | 方法 | 路径 | 说明 |
|---|---|---|---|
| 平台 | POST | `/api/auth/desktop/initiate` | 发起免密登录，返回一次性 `request_id`（60s，需平台签名） |
| 平台 | GET | `/api/auth/desktop/poll?request_id=` | 轮询状态：`pending`/`confirmed`/`used`/`expired`（需平台签名） |
| 平台 | POST | `/api/auth/desktop/exchange` `{request_id}` | 确认后兑换平台 token（校验授权，一次性，需平台签名） |
| 客户端 | POST | `/api/auth/desktop/login` | 客户端登录（账号/密码或 TOTP，同 verify 凭证格式），返回 `desktop_token`（30 天，仅返回一次） |
| 客户端 | POST | `/api/auth/desktop/confirm` `{desktop_token, request_id}` | 用户确认后绑定该请求与客户端用户 |

客户端本地服务（`127.0.0.1:5712`，仅本机）：`GET /identity`（平台探测会话用户）、`POST /pending`（平台推送确认请求）、`GET /status`（当前待确认状态）。

### 8.3 平台侧接入步骤

> **前后端分工**：带 secret 的认证中心调用（initiate/poll/exchange）由**平台后端**做；客户端本地探测与推送（`127.0.0.1:5712`）**必须在平台前端 JS 做**（客户端运行在用户浏览器所在电脑，后端访问不到）。
> 完整可照抄示例（Flask 后端 + 登录页 JS + 时序）见 `desktop/examples/platform_integration.md`；后端纯脚本版见 `desktop/examples/platform_desktop_login.py`。

```python
# —— 平台后端：三步签名调用（secret 只留在后端）——
# ① 发起
r = signed_request("POST", "/api/auth/desktop/initiate", {})
request_id = r["data"]["request_id"]          # 60s 有效，交给前端

# ② 轮询（前端 5s 一次轮询本接口）
r = signed_request("GET", f"/api/auth/desktop/poll?request_id={request_id}")
status = r["data"]["status"]                  # pending / confirmed / used / expired

# ③ 兑换平台 token（一次性）
r = signed_request("POST", "/api/auth/desktop/exchange", {"request_id": request_id})
token = r["data"]["token"]
```

```js
// —— 平台前端：探测 + 推送（跨域到用户本机 5712，CORS 已开）——
const id = await fetch('http://127.0.0.1:5712/identity', { mode: 'cors' }).then(r => r.json());
if (id.logged_in) {
  await fetch('http://127.0.0.1:5712/pending', {
    method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ request_id, platform: PLATFORM_ID }),   // → 客户端弹确认窗
  });
}
```
...
- **确认必须在客户端弹出**：平台网页只能探测客户端是否存在与当前用户，无法替用户确认；`request_id` 一次性（confirmed→used），60s 过期。
- **客户端身份**：`desktop/confirm` 凭 `desktop_token`（客户端登录时签发，认证中心只存哈希、可吊销）证明"当前桌面用户"，认证中心校验会话有效后才绑定请求。
- **平台仍要验授权**：`exchange` 时认证中心强制校验该用户对该平台的授权（未授权返回 1006），用户被禁用/删除即时拦截。
- **本地端口安全**：`127.0.0.1:5712` 仅本机可达；`/identity` 只返回登录状态（**不暴露用户信息**，防任意网页身份探测）；`/pending` 校验 request_id 格式（32 位十六进制，伪造的会在 confirm 时被认证中心拒绝）并按 1s 节流防弹窗轰炸。即使恶意网页探测到客户端并伪造推送，最终放行必须用户在客户端点击确认，且确认的请求必须在认证中心真实存在（平台签名 initiate 签发）。
- **desktop_token 保管**：存于客户端本机（`%APPDATA%/authplatform-desktop/session.json`），建议生产环境升级为系统 keyring 加密存储。
