...
### 8.3 平台侧接入步骤（Python 示例见 `desktop/examples/platform_desktop_login.py`）

```python
# 1) 发起：拿到 request_id（平台签名，见 §2）
r = signed_request("POST", "/api/auth/desktop/initiate")
request_id = r["data"]["request_id"]          # 60s 有效

# 2) 探测客户端并推送（前端 JS 或后端均可，客户端返回带 CORS）
identity = requests.get("http://127.0.0.1:5712/identity", timeout=2).json()
if identity["logged_in"]:
    requests.post("http://127.0.0.1:5712/pending",
                  json={"request_id": request_id, "platform": PLATFORM_ID})

# 3) 轮询确认（5s 一次，60s 内）
while True:
    st = signed_request("GET", f"/api/auth/desktop/poll?request_id={request_id}")["data"]["status"]
    if st == "confirmed": break
    if st in ("expired", "used"): raise TimeoutError(st)

# 4) 兑换平台 token（一次性）
token = signed_request("POST", "/api/auth/desktop/exchange", {"request_id": request_id})["data"]["token"]
```
